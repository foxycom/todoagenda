...
espresso-tests.log : app-original.apk app-androidTest.apk running-emulator
	...
	$(ADB) shell am instrument -w -r org.andstatus.todoagenda.tests/androidx.test.runner.AndroidJUnitRunner | tee espresso-tests.log
	export ABC_CONFIG=$(ABC_CFG) && $(ABC) stop-all-emulators
	$(RM) running-emulator
...
%.testlog: app-androidTest.apk app-instrumented.apk running-emulator
	$(eval FIRST_RUN := $(shell $(ADB) shell pm list packages | grep -c org.andstatus.todoagenda))
	@if [ "$(FIRST_RUN)" == "2" ]; then \
		echo "Resetting the data of the apk"; \
		$(ADB) shell pm clear org.andstatus.todoagenda; \
	else \
	...
	$(ADB) shell am instrument -w -e class $(TEST_NAME) org.andstatus.todoagenda.tests/androidx.test.runner.AndroidJUnitRunner | tee $(@)
...
copy-shadows :
	cp -v ./shadows/*.java app/src/carvedTest/org/andstatus/todoagenda
