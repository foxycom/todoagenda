...
JAVA_OPTS=" -Dabc.instrument.array.operations -Dabc.instrument.fields.operations -Dabc.taint.android.intents -Dabc.instrument.include=org.andstatus.todoagenda"
...
if [ "$$IS_RUNNING" == "0" ]; then \
        ...
        APP_INSTALLED=$$($(ADB) shell pm list packages | grep -c "org.andstatus.todoagenda$$"); \
        INSTRUMENTATION_TESTS_INSTALLED=$$($(ADB) shell pm list packages | grep -c "org.andstatus.todoagenda.test$$"); \
        if [ "$$APP_INSTALLED" == "1" ]; then \
				(>&2 echo "Removing app"); \
                $(ADB) uninstall org.andstatus.todoagenda > /dev/null; \
        else \
                (>&2 echo "App not installed"); \
        fi; \
        if [ "$$INSTRUMENTATION_TESTS_INSTALLED" == "1" ]; then \
				(>&2 echo "Removing test app"); \
                $(ADB) uninstall org.andstatus.todoagenda.test > /dev/null; \
        else \
...
$(shell if [ "$(TRACING_APKS_INSTALLED)" == "0" ]; then \
	...
	$(ADB) shell pm clear org.andstatus.todoagenda > /dev/null; \
fi)
...
app-original.apk : 
	...
	mv app/build/outputs/apk/debug/TodoAgenda-4.4.0-debug.apk app-debug.apk && \
...
app-androidTest.apk :
	...
	mv app/build/outputs/apk/androidTest/debug/TodoAgenda-4.4.0-debug-androidTest.apk app-androidTest-unsigned.apk && \
...
app-original-for-coverage.apk app-androidTest-for-coverage.apk:
	...
	mv app/build/outputs/apk/debug/TodoAgenda-4.4.0-debug.apk app-original-for-coverage.apk  && \
	mv app/build/outputs/apk/androidTest/debug/TodoAgenda-4.4.0-debug-androidTest.apk app-androidTest-for-coverage.apk
